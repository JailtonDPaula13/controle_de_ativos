...
begin
    ...
    select cpf into v_cpfc from tb_colaboradores where cpf = v_cpf;
	...
	if(length(v_nome_rs) > 40) then
        ...
        insert into tb_colaboradores values (sq_colaboradores.nextval, upper(v_nome_rs), upper(v_nome), upper(v_sexo), v_cpf, v_id_setor);
	end if; 
end;